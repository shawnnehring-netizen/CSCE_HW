...
// helper function "mstoi" to converts MyString to int, works a lot like number slicing in reverse
int mstoi(const MyString& str) {  // just doing base 10 here
    int result = 0;
    ...
    while (!commands.eof()) {
        ...
        switch (comm.front()) {
            case 'S':
                ...
                break;
            case 'R':
                testRover.read(mstoi(data));  // mstoi is provided above
                break;
            case 'J':
                testRover.join(data);
                break;
